...
#### No.S3: Testing the performance of normal CCM analysis using simulation data
####
...
dir.create(config$out.09.dir, showWarnings = FALSE)
...
Erange <- config$k09Erange
###############################

repID.i <- 1 # ID to identify replication
sink(config$out.09.sim.conditions.path)
...
for (force.seasonality.i in force.seasonality.seq) {
  ...
  for (error.rate.i in error.rate.seq) {
    for (rep.i in 1:sim.replication) {
      ...
      sim.res.tmp <- FourierOriCcmSim(parms.list)
  ...
  csv.name <- config$Out09SimResPath(force.seasonality.i)
...
#### save image
save.image(file = config.09$out.09.rdata.path)

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_0_SessionInfo/S03_SessionInfo_SimOriCCM_%s.txt", substr(Sys.time(), 1, 10)))
