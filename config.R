...
config <- local(function() {
  ...
  kOut05FFTRDataFile <- '05_FFT_LagCCM_FFT.RData'
  ...
  for (name in kSiteNamesClimate) {
    out.002.site.dir[[name]] <- file.path(base.dir, kOut002Dir, kOut002SubDir, sprintf(kOut002SubSiteDir, name))
  ...
  Out05FFTFigsDir <- function(site.name){
    sprintf(file.path(out.05.dir, kOut05FFTFigsDir, kOut05FigsSubDir), site.name)
  }
  
  Out05FFTPdfPath <- function(site.name, site.name.sub, litter, climate, ccm.tp){
      sprintf(file.path(Out05FFTFigsDir(site.name), kOut05PdfFile),
        site.name.sub, litter, climate, ccm.tp)
  }
  ...
  CausalityCriteria <- function(ccm.out) {
    ...
    cond3 <- ccm.out$tp < 1
    list(cond = cond1 &
           cond3, comment = "combined p-value < 0.05") # 2018.6.11
  }
