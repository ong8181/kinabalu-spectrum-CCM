...
#### Figure code for Supplementary Information: Performance test of spectrum-CCM analysis using simulation data
####
...
for(season_i in seq(0, 2, by = 0.2)){
  sim_file_name <- sprintf("../S02_out/S02_Fourier_SimRes_Seasonality_%s.csv", season_i)
...
quartz(type = "pdf", file = "0_RawFigs/0_archives/FigS_fCCMtest_Pval.pdf", width = 10, height = 3)
comb.g1
dev.off()

quartz(type = "pdf", file = "0_RawFigs/0_archives/FigS_fCCMtest_FDR.pdf", width = 10, height = 3)
comb.g2
dev.off()

quartz(type = "pdf", file = "0_RawFigs/0_archives/FigS_fCCMtest_Contour.pdf", width = 11, height = 3)
comb.g3
dev.off()


# Combined figure output
quartz(type = "pdf", file = "0_RawFigs/FigS_fCCMtest_All.pdf", width = 10, height = 9)
plot_grid(comb.g1, comb.g2, comb.g3, ncol = 1, align = "hv")
dev.off()
